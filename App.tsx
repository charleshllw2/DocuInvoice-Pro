...
const App: React.FC = () => {
  ...
  const handleLogin = async () => {
    try {
      ...
      setIsAuthenticated(true); // Fallback for review purposes
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      alert('Failed to generate document. Please check console.');
    } finally {
    ...
    <div className="h-screen flex flex-col bg-gray-50 overflow-hidden">
      ...
      <header className="bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between shrink-0 h-16 z-20 shadow-sm">
        ...
        <div className="flex items-center gap-4">
          ...
          <span className="text-sm text-gray-500 hidden lg:inline border-r border-gray-200 pr-4 mr-1">Demo User</span>
      ...
      <main className="flex-1 flex overflow-hidden relative">
        ...
        {currentView === 'dashboard' ? (
          ...
          <>
            ...
            <div className="lg:hidden absolute top-0 left-0 right-0 flex border-b border-gray-200 bg-white shrink-0 z-30">
              <button
                ...
                className={`flex-1 py-3 text-sm font-medium flex items-center justify-center gap-2 transition-colors ${
                  mobileTab === 'editor' ? 'text-brand-600 border-b-2 border-brand-600' : 'text-gray-500'
                }`}
              >
              ...
              <button
                ...
                className={`flex-1 py-3 text-sm font-medium flex items-center justify-center gap-2 transition-colors ${
                  mobileTab === 'preview' ? 'text-brand-600 border-b-2 border-brand-600' : 'text-gray-500'
                }`}
              >
                <Eye size={16} /> Preview
              </button>
            ...
            {/* Left Panel: Form */}
            <div 
              className={`w-full lg:w-5/12 bg-white border-r border-gray-200 flex flex-col shadow-xl z-10 transition-transform duration-300 absolute inset-0 lg:static mt-11 lg:mt-0 ${
            ...
            {/* Right Panel: Preview */}
            <div 
              className={`w-full lg:flex-1 bg-gray-100 flex items-start justify-center relative absolute inset-0 lg:static mt-11 lg:mt-0 transition-transform duration-300 ${
